...
const EDIT_COLS = [
  'milestone', 'activities', 'type', 'status', 'owner',
  'duration', 'planned_start', 'planned_end', 'actual_start', 'current_end',
  'dependency', 'notes',
];
...
const COL_DEFS = [
  { key: 'milestone',    label: 'Milestone',     dbKey: 'milestone'  },
  { key: 'activities',   label: 'Activities',    dbKey: 'activities' },
  { key: 'type',         label: 'Type',          dbKey: 'tools'      },
  { key: 'status',       label: 'Status',        dbKey: 'status'     },
  { key: 'owner',        label: 'Owner',         dbKey: 'owner'      },
  { key: 'duration',     label: 'Actual Days',   dbKey: 'duration'   },
  { key: 'planned_start', label: 'Planned Start', dbKey: 'planned_start' },
  { key: 'planned_end',  label: 'Planned End',   dbKey: 'planned_end'  },
  { key: 'actual_start', label: 'Actual Start',  dbKey: 'actual_start' },
  { key: 'current_end',  label: 'Actual End',    dbKey: 'current_end'  },
  { key: 'dependency',   label: 'Dependency',    dbKey: 'dependency'   },
  { key: 'notes',        label: 'Comments',      dbKey: 'learnings'    },
];
...
// Map colKey → db field for reading
const colKeyToDb = COL_DEFS.reduce((acc, c) => { acc[c.key] = c.dbKey; return acc; }, {});
...
export default function DeliveryPlanTab({ project, canEdit }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const [taskData, peopleData] = await Promise.all([getPlanTasks(project.id), getPeople(project.id)]);
        setTasks(taskData || []);
  ...
  // handleCellChange uses the db-field key directly
  const handleCellChange = useCallback((taskId, dbField, value) => {
    setTasks(prev => {
      const next = prev.map(t => t.id === taskId ? { ...t, [dbField]: value } : t);
  ...
  const handleAddRow = useCallback(() => {
    const newTask = {
      ...
      milestone: '', activities: '', tools: 'Ext', status: 'Not Started',
      owner: '', duration: null, planned_start: null, planned_end: null,
      actual_start: null, current_end: null, dependency: '', learnings: '',
      sort_order: Date.now(),
    };
  ...
  const handleExport = useCallback(() => {
    ...
    const rows = tasks.map(t => COL_DEFS.map(c => t[c.dbKey] ?? ''));
    ...
    <div className="space-y-4 p-6" onClick={() => ss.clearAll()}>
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Click to select · Click again to edit · Enter moves down · Tab moves right</p>
        </div>
      ...
      <div className="overflow-x-auto border border-gray-300 rounded-lg select-none" onClick={e => e.stopPropagation()}>
        <table className="border-collapse text-xs" style={{ minWidth: '100%' }}>
          ...
          <tbody>
            {tasks.map((task, idx) => {
              const mColor = task.milestone ? getMilestoneColor(task.milestone, milestoneColorMap.current) : null;
              ...
              return (
                <tr key={task.id}
                  ...
                  {/* Milestone — colored background */}
                  <SCell ss={ss} rowId={task.id} colKey="milestone"
                    value={task.milestone || ''}
                    onChange={v => handleCellChange(task.id, 'milestone', v)}
                    rows={tasks} cols={EDIT_COLS}
                    disabled={!canEdit}
                    tdStyle={{ ...cw('milestone'), backgroundColor: mColor || undefined }}
                    readView={<span className="text-xs font-medium text-gray-700">{task.milestone || '—'}</span>} />

                  {/* Activities */}
                  <SCell ss={ss} rowId={task.id} colKey="activities"
                    value={task.activities || ''}
                    onChange={v => handleCellChange(task.id, 'activities', v)}
                    rows={tasks} cols={EDIT_COLS}
                    disabled={!canEdit} tdStyle={cw('activities')} />

                  {/* Type (stored in tools field) */}
                  <SCell ss={ss} rowId={task.id} colKey="type"
                    value={task.tools || ''}
                    onChange={v => handleCellChange(task.id, 'tools', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="colored-select" options={TYPE_OPTIONS} colorMap={TYPE_COLORS}
                    disabled={!canEdit} tdStyle={cw('type')}
                    readView={
                  ...
                  {/* Status */}
                  <SCell ss={ss} rowId={task.id} colKey="status"
                    value={task.status || ''}
                    onChange={v => handleCellChange(task.id, 'status', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="colored-select" options={STATUS_OPTIONS} colorMap={STATUS_COLORS}
                  ...
                  {/* Owner */}
                  <SCell ss={ss} rowId={task.id} colKey="owner"
                    value={task.owner || ''}
                    onChange={v => handleCellChange(task.id, 'owner', v)}
                    rows={tasks} cols={EDIT_COLS}
                    disabled={!canEdit} tdStyle={cw('owner')} />

                  {/* Duration / Actual Days */}
                  <SCell ss={ss} rowId={task.id} colKey="duration"
                    value={task.duration != null ? String(task.duration) : ''}
                    onChange={v => handleCellChange(task.id, 'duration', v ? parseInt(v) : null)}
                    rows={tasks} cols={EDIT_COLS}
                    type="number"
                    disabled={!canEdit} tdStyle={cw('duration')} tdClass="text-right"
                    readView={<span className="ml-auto text-xs">{task.duration ?? '—'}</span>} />

                  {/* Planned Start */}
                  <SCell ss={ss} rowId={task.id} colKey="planned_start"
                    value={task.planned_start || ''}
                    onChange={v => handleCellChange(task.id, 'planned_start', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="date" disabled={!canEdit} tdStyle={cw('planned_start')} />

                  {/* Planned End */}
                  <SCell ss={ss} rowId={task.id} colKey="planned_end"
                    value={task.planned_end || ''}
                    onChange={v => handleCellChange(task.id, 'planned_end', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="date" disabled={!canEdit} tdStyle={cw('planned_end')} />

                  {/* Actual Start */}
                  <SCell ss={ss} rowId={task.id} colKey="actual_start"
                    value={task.actual_start || ''}
                    onChange={v => handleCellChange(task.id, 'actual_start', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="date" disabled={!canEdit} tdStyle={cw('actual_start')} />

                  {/* Actual End (stored in current_end) */}
                  <SCell ss={ss} rowId={task.id} colKey="current_end"
                    value={task.current_end || ''}
                    onChange={v => handleCellChange(task.id, 'current_end', v)}
                    rows={tasks} cols={EDIT_COLS}
                    type="date" disabled={!canEdit} tdStyle={cw('current_end')} />

                  {/* Dependency */}
                  <SCell ss={ss} rowId={task.id} colKey="dependency"
                    value={task.dependency || ''}
                    onChange={v => handleCellChange(task.id, 'dependency', v)}
                    rows={tasks} cols={EDIT_COLS}
                    disabled={!canEdit} tdStyle={cw('dependency')} />

                  {/* Comments (stored in learnings) */}
                  <SCell ss={ss} rowId={task.id} colKey="notes"
                    value={task.learnings || ''}
                    onChange={v => handleCellChange(task.id, 'learnings', v)}
                    rows={tasks} cols={EDIT_COLS}
                    disabled={!canEdit} tdStyle={cw('notes')} />

                  {/* Row resize handle */}
                  <td
                    style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 4, cursor: 'row-resize', zIndex: 5, display: 'block' }}
                    onMouseDown={e => {
                      e.preventDefault();
      ...
      <p className="text-[10px] text-gray-400 mt-2">
        Drag column borders to resize · Drag row borders to resize rows · Type is Ext (External) or Int (Internal)
      </p>
    </div>
  );
}
